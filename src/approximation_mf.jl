...
function get∇̂³𝝭(ap::ReproducingKernel,ξ::Node)
    𝒙 = get𝒙(ap,ξ)
    return get∇̂³𝝭(ap,𝒙)
end
...
function get∇̂³𝝭(ap::ReproducingKernel,𝒙::NTuple{3,Float64})
    𝓒 = ap.𝓒
    𝝭 = ap.𝝭[:∂1]
    ∂𝝭∂x = ap.𝝭[:∂x]
    ∂𝝭∂y = ap.𝝭[:∂y]
    ...
    for (i,𝒙ᵢ) in enumerate(𝓒)
        ...
        𝝭[i] = 𝒑₀ᵀ𝗠⁻¹*𝒑*𝜙
        for j in 1:n
            ∂𝝭∂x[i] -= 𝒑₀ᵀ𝗠⁻¹[2,j]*𝒑[j]*𝜙
            ∂𝝭∂y[i] -= 𝒑₀ᵀ𝗠⁻¹[3,j]*𝒑[j]*𝜙
            ∂²𝝭∂x²[i] += 2*𝒑₀ᵀ𝗠⁻¹[4,j]*𝒑[j]*𝜙
            ∂²𝝭∂x∂y[i] += 𝒑₀ᵀ𝗠⁻¹[5,j]*𝒑[j]*𝜙
            ∂²𝝭∂y²[i] += 2*𝒑₀ᵀ𝗠⁻¹[6,j]*𝒑[j]*𝜙
            ∂³𝝭∂x³[i] -= 6*𝒑₀ᵀ𝗠⁻¹[7,j]*𝒑[j]*𝜙
            ∂³𝝭∂x²∂y[i] -= 2*𝒑₀ᵀ𝗠⁻¹[8,j]*𝒑[j]*𝜙
            ∂³𝝭∂x∂y²[i] -= 2*𝒑₀ᵀ𝗠⁻¹[9,j]*𝒑[j]*𝜙
            ∂³𝝭∂y³[i] -= 6*𝒑₀ᵀ𝗠⁻¹[10,j]*𝒑[j]*𝜙
        end
    end
    return 𝝭, ∂𝝭∂x, ∂𝝭∂y, ∂²𝝭∂x², ∂²𝝭∂x∂y, ∂²𝝭∂y², ∂³𝝭∂x³, ∂³𝝭∂x²∂y, ∂³𝝭∂x∂y², ∂³𝝭∂y³
end
...
function set∇̂³𝝭!(aps::Vector{T}) where T<:ReproducingKernel{SNode}
    for ap in aps
        set∇̂³𝝭!(ap)
    end
end
...
function set∇̂³𝝭!(ap::ReproducingKernel{SNode})
    𝓒 = ap.𝓒
    𝓖 = ap.𝓖
    for ξ in 𝓖
        i = ξ.id
        I = ξ.index[i]
        ξ̂ = Node(ξ)
        𝝭,∂𝝭∂x,∂𝝭∂y,∂²𝝭∂x²,∂²𝝭∂x∂y,∂²𝝭∂y²,∂³𝝭∂x³,∂³𝝭∂x²∂y,∂³𝝭∂x∂y²,∂³𝝭∂y³ = get∇̂³𝝭(ap,ξ̂)
        for j in 1:length(𝓒)
            ξ.𝝭[:∂1][I+j] = 𝝭[j]
            ξ.𝝭[:∂x][I+j] = ∂𝝭∂x[j]
            ξ.𝝭[:∂y][I+j] = ∂𝝭∂y[j]
