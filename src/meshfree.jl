...
function cal𝗠ₕ!(ap::ReproducingKernel{:Linear2D},x::SNode)
    𝓒 = ap.𝓒
    𝗠 = get𝗠(ap,:𝗠)
    for xᵢ in 𝓒
        Δx = x - xᵢ
        ...
        for I in 1:3
            for J in 1:I
                𝗠[I,J] += 𝜙*(𝒑[I]*𝒑[J]+∂𝒑∂x[I]*∂𝒑∂x[J]+∂𝒑∂y[I]*∂𝒑∂y[J])
            end
        end
    end
    cholesky!(𝗠)
...
function cal𝗚!(ap::ReproducingKernel{:Linear2D,𝑠,𝜙,:Vor2}) where {𝑠,𝜙}
    𝓒 = ap.𝓒
    𝓖 = ap.𝓖
    𝗚 = get𝗚(ap,:∇̃)
    n = get𝑛𝒑₁(ap)
...
function cal𝗚!(ap::ReproducingKernel{:Quadratic2D,𝑠,𝜙,:Vor2}) where {𝑠,𝜙}
    𝓒 = ap.𝓒
    𝓖 = ap.𝓖
    𝗚 = get𝗚(ap,:∇̃)
    n = get𝑛𝒑₁(ap)
...
function set𝝭ₕ!(ap::ReproducingKernel{:Linear2D},𝒙::SNode)
    𝓒 = ap.𝓒
    𝝭 = 𝒙[:𝝭]
    𝝭₁ = 𝒙[:𝝭₁]
    𝝭₂ = 𝒙[:𝝭₂]
...
function set∇𝝭ₕ!(ap::ReproducingKernel{:Linear2D},𝒙::SNode)
    𝓒 = ap.𝓒
    n = get𝑛𝒑(ap)
    𝝭 = 𝒙[:𝝭]
    𝝭₁ = 𝒙[:𝝭₁]
    ...
    ∂𝝭₁∂y = 𝒙[:∂𝝭₁∂y]
    ∂𝝭₂∂y = 𝒙[:∂𝝭₂∂y]
    𝒑₀,∂𝒑₀∂x,∂𝒑₀∂y= get∇₂𝒑(ap,(0.0,0.0,0.0))
    𝒑₀ᵀ𝗠⁻¹ = cal𝗠ₕ!(ap,𝒙)
    for (i,𝒙ᵢ) in enumerate(𝓒)
        Δ𝒙 = 𝒙 - 𝒙ᵢ
        𝒑,∂𝒑∂x,∂𝒑∂y= get∇₂𝒑(ap,Δ𝒙)
        ...
        for j in 1:n
            ∂𝝭∂x_ += 𝒑₀ᵀ𝗠⁻¹[2,j]*𝒑[j]*𝜙
            ∂𝝭₁∂x_ += 𝒑₀ᵀ𝗠⁻¹[2,j]*∂𝒑∂x[j]*𝜙
            ∂𝝭₂∂x_ += 𝒑₀ᵀ𝗠⁻¹[2,j]*∂𝒑∂y[j]*𝜙
            ∂𝝭∂y_ += 𝒑₀ᵀ𝗠⁻¹[3,j]*𝒑[j]*𝜙
            ∂𝝭₁∂y_ += 𝒑₀ᵀ𝗠⁻¹[3,j]*∂𝒑∂x[j]*𝜙
            ∂𝝭₂∂y_ += 𝒑₀ᵀ𝗠⁻¹[3,j]*∂𝒑∂y[j]*𝜙
        end
        ∂𝝭∂x[i] = ∂𝝭∂x_
        ∂𝝭₁∂x[i] = ∂𝝭₁∂x_
        ∂𝝭₂∂x[i] = ∂𝝭₂∂x_
...
function set∇̃𝝭!(gp::ReproducingKernel{𝒑,𝑠,𝜙,:Vor2},ap::ReproducingKernel{𝒑,𝑠,𝜙,:Vor2}) where {𝒑,𝑠,𝜙}
    𝓒 = gp.𝓒
    𝓖 = gp.𝓖
    x = 𝓖[1]
    for ξ̂ in 𝓖
...
function set∇̃𝝭!(cp::ReproducingKernel{𝒑,𝑠,𝜙,:Vor2},gp::ReproducingKernel{𝒑,𝑠,𝜙,:Vor2},ap::ReproducingKernel{𝒑,𝑠,𝜙,:Vor2}) where {𝒑,𝑠,𝜙}
    𝓒 = cp.𝓒
    𝓖 = cp.𝓖
    x = gp.𝓖[1]
    for ξ̂ in 𝓖
...
function set∇̄𝝭!(ap::ReproducingKernel{𝒑,𝑠,𝜙,:Vor2}) where {𝒑,𝑠,𝜙}
    𝓒 = ap.𝓒
    𝓖 = ap.𝓖
    for ξ̂ in 𝓖
        𝒒̂ = get𝒑₁(ap,ξ̂.x,ξ̂.y)
        𝗚⁻¹ = cal𝗚!(ap)
        𝒒̂ᵀ𝗚⁻¹ = 𝒒̂*𝗚⁻¹
        ∂𝝭∂x = ξ̂[:∂𝝭∂x_]
        ∂𝝭∂y = ξ̂[:∂𝝭∂y_]
...
function set∇̄𝝭!(ap::ReproducingKernel)
    𝓒 = ap.𝓒
    𝓖 = ap.𝓖
    for ξ̂ in 𝓖
        𝒒̂ = get𝒑₁(ap,ξ̂)
